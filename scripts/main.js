...
const game = {
    ...
    drawSnake() {
        ...
        this.snake.forEach((element, index) => {
            ...
            const snakePartElement = $(`.snake-part.id-${index}`);
            ...
            const borderHandler = (snakePart, snakePartNext) => {
                if (snakePart.x === snakePartNext.x) {
                    if (snakePart.y === snakePartNext.y + 1) {
                        borderTop = false;
                    } else if (snakePart.y === snakePartNext.y - 1) {
                        borderBottom = false;
                    } else if (snakePart.y === 0) {
                        borderTop = false;
                    } else if (snakePartNext.y === 0) {
                        borderBottom = false;
                    }
                } else if (snakePart.y === snakePartNext.y) {
                    if (snakePart.x === snakePartNext.x + 1) {
                        borderLeft = false;
                    } else if (snakePart.x === snakePartNext.x - 1) {
                        borderRight = false;
                    } else if (snakePart.x === 0) {
                        borderLeft = false;
                    } else if (snakePartNext.x === 0) {
                        borderRight = false;
                    }
                }
            };

            if (this.snake[index + 1]) {
                borderHandler(this.snake[index], this.snake[index + 1]);
            }
            if (this.snake[index - 1]) {
                borderHandler(this.snake[index], this.snake[index - 1]);
            }

            Object.assign(snakePartElement.style, {
